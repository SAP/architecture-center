...
export async function findAndSaveDrawioFiles(node: any, drawioFolderPath: string) {
    if (!node) return;

    if (node.type === 'drawio' && node.diagramXML) {
        const fileName = `diagram-${nanoid(10)}.drawio`;
        const filePath = path.join(drawioFolderPath, fileName);
        await fs.writeFile(filePath, node.diagramXML);
        node.fileName = fileName;
    }

    if (node.children && Array.isArray(node.children)) {
        for (const child of node.children) {
            await findAndSaveDrawioFiles(child, drawioFolderPath);
        }
    }
}
...
export function convertLexicalToMarkdown(editorState: string | null): string {
    if (!editorState) {
        return '';
    }
    try {
