...
-  SAP BW NetWeaver to SAP Business Warehouse, private cloud edition( SAP BW 7.5 on HANA) to benefit from maintenance support until end of 2030
...
- **SAP Analytics Cloud:** Provides Analytics & BI, Enterprise Planning.
- **SAP Databricks:** Supports AI/ML scenarios on unified SAP BW and SAP BDC data.
- **Data Products**: Data set exposed for consumption outside the boundaries of the producing application via APIs and described by high quality metadata that can be accessed through the Data Product Directory.
...
**The key benefits of SAP Business Warehouse, private cloud edition for SAP NetWeaver as part of SAP BDC are:**
...
- Upgrade to BW 7.5 on HANA database (in case of any DB) and check if SPS24 is fulfilled or upgrade to latest SPS.

**For SAP BW 7.5 deployments:** 
- Upgrade to HANA database (in case of any DB) and check if SPS24 is fulfilled or upgrade to latest SPS.

**For Existing SAP BW/4HANA deployments** 

- SAP BW/4HANA 1.0 and 2.0: Upgrade to SAP BW/4HANA 2023 latest support package.
- SAP BW/4HANA 2021: Check if SPS04 is fulfilled or upgrade to latest SPS.
...
-   **Primary data product** contains the flattened transactional data of the selected InfoProvider including master data which can be e.g. directly leveraged by SAP Databricks for ML/AI use cases via Delta Share.
...
With data product generator, a data subscription is created in SAP BW/4HANA Cockpit or via SAPGUI for 7.5 for InfoProvider. Once the subscription is activated, Local Table (File) is created in a specific BW Inbound space in SAP Datasphere. Multiple subscriptions can be created similarly.

Once the run subscription is executed, the data from the InfoProvider gets loaded into Local Table (File) into the BW inbound space. Objects in this space are read-only, but data management tasks (e.g. deleting data) are possible. A merge tasks takes care of data consistency between the BW source and the Local Table Files in SAP Datasphere. Update of data can be scheduled in a delta mode as well.

To create a data product, a data provider profile is required which can be created in the Data Sharing Cockpit in SAP Datasphere. Existing data provider profiles can be re-used by generating activation keys and distributing them accordingly. In order to make data products consumable across SAP BDC components, the data products need to be exposed to the formation.

Data products can be created specifying the artifact space (BW Inbound Space) and other required details. Single or multiple tables can be added to the data product. To make this discoverable in the Catalog, change the Switch Status to 'List'. This triggers the creation of a delta share endpoint and an ORD document that will be shared to Unified Customer Landscape. Data products will be listed in the Catalog from where this can be shared to other spaces.

We can select one or multiple tables which should be exposed through the data product.

With this, BW data products will be available for modelling and transformation purposes in SAP BDC. Leveraging delta share with SAP Databricks, proven BW Data can be used for implementing AI/ML use cases. 
...
With the data products in SAP BDC, holistic integration between data platforms and business applications is possible. This helps to leverage Intelligent applications from SAP for insights and business decisions. These intelligent applications can be customized at analytical model and views layer.

![drawio](drawio/bw-approach-3.drawio)

Along with SAP BW data product and all other LoB data products, one of the approaches for building AI/ML applications can be achieved using (SAP) Databricks - SAP Databricks and Native Databricks. SAP Databricks is optimized to work with SAP data product with zero-copy bi-directional data product sharing.

From SAP Datasphere's Catalog, the BW data product can be shared with (SAP) Databricks. Once the AI/ML analysis has been performed in SAP Databricks, the data from the table in SAP Databricks will be shared back via Catalog and in SAP Datasphere where we can harmonize it with other data from other sources.

Refer to [SAP Business Data Cloud SDK](https://pypi.org/project/sap-bdc-connect-sdk/) to be able to create and publish data product for downstream consumption within SAP BDC. This SDK helps to create/update share, create/update the CSN for a share and publish/unpublish a data product.

For customers who are already using Databricks Data Intelligence Platform, BDC Connect (Partner Connector) from SAP help integration for brownfield scenarios. This will enable zero-copy sharing of data products bi-directionally based on Delta Sharing. 


**Replacing SAP BW Use Cases with SAP Business Data Cloud**

With SAP BDC, SAP BW use cases can be gradually transitioned with SAP BDC with SAP-managed data products and Intelligent Applications.
...
-   Replace Standard Data Sources with SAP managed data product.
...
-   **Transformation Effort:** No migration cost for upgrade to SAP BW/4HANA. Leverage SAP-managed data products with business semantics helping to reduce data integration and maintenance cost.
-   **Analytics and Tech Support Cost:** Reduction in annual monitoring, technical patching and upgrade cost and reducing losses from unforeseen data risk.
-   **SAP Software & Maintenance:** Minimize or eliminate SAP Datasphere Premium Outbound cost by leveraging zero-copy approach. Eliminate SAP BW, private cloud edition cost after decommissioning.
