...
Steps 1-3 are one-time project setup. Steps 4-8 are the repeating development cycle, executing once per feature and looping until the feature merges to main.
...
2.  **SAP Knowledge Activation:** The developer connects the coding agent to the SAP MCP servers on BTP (client-managed). These servers, covering CAP, Fiori Elements, and UI5, expose current SAP framework knowledge as callable tools. Specialized SAP development skills invoke the appropriate MCP server semantically during generation and code validation.
3.  **Specification and Grounding:** Using a Spec-Driven-Development tool (i.e.: superpowers) configured with project context and architecture principles, **curated specification** in markdown format are co-created by humans and AI containing requirements, test cases and acceptance criteria. The specification captures software development practices and non-functional constraints, including security posture, performance budgets, reliability expectations, scalability limits, that the generated code must satisfy.
4.  **Task Decomposition:** The coding agent decomposes the specifications into a structured plan with tasks and their phases in markdown format. Dependency mappings determine which tasks can execute **concurrently** and which must wait, maximizing parallel code production across the agent team. Human approves the plan and the agent assigns each task to specialized agents, such as a backend agent responsible for CAP services and CDS models, and a frontend agent responsible for Fiori annotations and UI5 controllers, each operating in an isolated worktree.
...
6.  **Quality Gate Execution:** The quality pipeline executes the test suite, linters, security scans and browser-based verification against the generated output. Gates run against the full codebase, catching regressions in existing features alongside new code. Non-conforming code is rejected and returned to the agent for correction. No code advances without passing all gates.
7.  **Developer Review and Testing:** The developer is responsible for reviewing the consolidated pull request, running the application, and validating that the implemented behavior matches the acceptance criteria. A reviewer agent pre-screens the pull request before the developer sees it,  flagging code that does not trace to a spec requirement and producing a structured summary of changes and concerns, so the developer can focus review on what matters. Rejected work returns to the agents with the developer's feedback captured in the specification, re-entering the production cycle at Step 5.
8.  **Merge to Main:** Once the developer's integration branch is fully reviewed and tested, the developer submits a pull request to the project's main branch. Branch protection rules require passing automated gates and an approved review before the merge is accepted. Each merged change carries a semantic commit with testing evidence and traceability to the original requirements.
