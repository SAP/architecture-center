...
1. **Set up Continuous Integration**: A key driver of the agility and built-in quality that DevOps brings is the automation of large parts of the deployment pipeline. You can quickly set up a Continuous Integration / Continuous Delivery (CI/CD) pipeline in the SAP Continuous Integration and Delivery service. Simply create a new job by selecting one of the provided pipeline templates for typical SAP BTP development use cases, such as developing a side-by-side extension on SAP BTP Cloud Foundry, or building compelling UIs for your ABAP Frontend Server. After connecting the pipeline to your source code management system and configuring the webhook, it is executed automatically whenever your development teams submit changes to their source code repository. This enables the pipeline to build and test their changes, providing direct feedback on the quality of their updates. This approach allows developers to benefit from an agile methodology with direct SAP support, requiring minimal expertise and no need to set up or operate their own CI/CD infrastructure. As a result, the cognitive load of your development teams is reduced. SAP Continuous Integration and Delivery is also part of SAP Build.
